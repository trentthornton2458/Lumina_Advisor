...
export default function SentimentDashboard({ notes, contacts, companies }: SentimentDashboardProps) {
  ...
  return (
    <div id="sentiment-analysis-dashboard" className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-5 flex items-center justify-between">
          <div>
            ...
            <span className="text-[10px] text-slate-500 block mt-1">General conversation tone</span>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-5 flex items-center justify-between">
          <div>
            ...
            <span className="text-[10px] text-slate-500 block mt-1">{relevantContacts.length} professional profiles</span>
          </div>
          <div className="p-3 bg-slate-50 text-slate-700 rounded-xl border border-slate-150">
        ...
        <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-5 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest block">Coaching Tips</span>
            <span className="text-2xl font-bold font-mono text-indigo-600 mt-1.5 inline-block">{coachingOpportunityCount}</span>
            <span className="text-[10px] text-slate-500 block mt-1">AI-flagged opportunities logged</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-[360px]">
          <div className="mb-3">
            <h3 className="text-sm font-semibold tracking-wider text-slate-500 uppercase">Sentiment by Meeting Category</h3>
        ...
        <div className="lg:col-span-5 bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-[360px] overflow-y-auto">
          <div className="mb-3">
            <h3 className="text-sm font-semibold tracking-wider text-slate-500 uppercase">Relationship Movers</h3>
          ...
          {movers.improved.length === 0 && movers.declined.length === 0 ? (
            ...
            <div className="space-y-4">
              {movers.improved.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-1.5 flex items-center gap-1"><ArrowUpRight size={12} /> Most Improved</h4>
                  <div className="space-y-1.5">
                    {movers.improved.map(m => (
                      <div key={m.contact.id} className="flex justify-between items-center text-xs bg-emerald-50 border border-emerald-150 rounded-lg px-2.5 py-1.5">
                        <span className="font-semibold text-slate-800 truncate">{m.contact.name}</span>
              ...
              {movers.declined.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-bold text-rose-600 uppercase tracking-wider mb-1.5 flex items-center gap-1"><ArrowDownRight size={12} /> Most Declined</h4>
                  <div className="space-y-1.5">
                    {movers.declined.map(m => (
                      <div key={m.contact.id} className="flex justify-between items-center text-xs bg-rose-50 border border-rose-150 rounded-lg px-2.5 py-1.5">
                        <span className="font-semibold text-slate-800 truncate">{m.contact.name}</span>
          ...
          {spotlightNotes.best && (
            <div className="bg-white rounded-xl border border-emerald-200 p-5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider flex items-center gap-1.5"><Star size={12} /> Best Logged Interaction</span>
                <span className="text-xs font-mono font-bold text-emerald-700 bg-emerald-50 border border-emerald-150 rounded-md px-2 py-0.5">{spotlightNotes.best.sentimentScore}/10</span>
              </div>
              <h4 className="font-semibold text-slate-900 text-sm">{spotlightNotes.best.title}</h4>
              <p className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1"><Building2 size={11} /> {noteAttendeeLabel(spotlightNotes.best)} &middot; {spotlightNotes.best.date}</p>
              <p className="text-xs text-slate-600 mt-2 leading-relaxed">{noteExcerpt(spotlightNotes.best)}</p>
            </div>
          )}
          {spotlightNotes.worst && (
            <div className="bg-white rounded-xl border border-rose-200 p-5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold text-rose-700 uppercase tracking-wider flex items-center gap-1.5"><AlertCircle size={12} /> Highest-Risk Interaction</span>
                <span className="text-xs font-mono font-bold text-rose-700 bg-rose-50 border border-rose-150 rounded-md px-2 py-0.5">{spotlightNotes.worst.sentimentScore}/10</span>
              </div>
              <h4 className="font-semibold text-slate-900 text-sm">{spotlightNotes.worst.title}</h4>
              <p className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1"><Building2 size={11} /> {noteAttendeeLabel(spotlightNotes.worst)} &middot; {spotlightNotes.worst.date}</p>
              <p className="text-xs text-slate-600 mt-2 leading-relaxed">{noteExcerpt(spotlightNotes.worst)}</p>
            </div>
          )}
        </div>
      )}
